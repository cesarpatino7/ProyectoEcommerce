...
const ProductCard = ({ id, name, image, price }) => {
  ...
  const handleCardClick = (e) => {
    // Si el clic fue en el área de cantidad o el botón, no navegamos
    if (e.target.closest('.quantity-control') || e.target.closest('.add-to-cart')) {
      return;
    }
    navigate(`/product/${id}`);
  };
  ...
  const handleAddToCart = (e) => {
    ...
    console.log(`Agregando ${quantity} unidad(es) del producto ${id} al carrito`);
  };
  ...
  const formatPrice = (price) => {
    ...
    }).format(price * 7300); // Conversión a guaraníes
  };
  ...
  return (
    ...
    >
      ...
      <div className="card-body items-center text-center p-4">
        ...
        <div className="quantity-control flex items-center gap-2 mt-2" onClick={e => e.stopPropagation()}>
          <button 
